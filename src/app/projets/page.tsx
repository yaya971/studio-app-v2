...
export default function ProjetsPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (window.confirm(t('proj.alert.delete_proj'))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.artiste_id) { alert(t('proj.alert.no_artist')); setIsSubmitting(false); return; }
    if (editingId) {
  ...
  const deleteChanson = async (id: string) => {
    if (window.confirm(t('proj.alert.delete_track'))) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, chansonId: string) => {
    ...
    try {
      ...
      fetchData();
    } catch (error: any) { alert(t('proj.alert.upload_err') + error.message); } 
    finally { setUploadingId(null); }
  };

  const deleteAudio = async (chansonId: string) => {
    if (window.confirm(t('proj.alert.delete_audio'))) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#4ade80] drop-shadow-[0_0_8px_rgba(74,222,128,0.8)]">{t('proj.title')}</h1>
          <p className="mt-2 text-gray-400 font-bold">{t('proj.subtitle')}</p>
        </div>
        {isAdmin && (
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-[#4ade80]/30 bg-black/50 p-8 text-center"><Folder className="mx-auto mb-4 text-[#4ade80]/50" size={48} /><h3 className="mb-2 text-xl font-bold text-white">{t('proj.empty')}</h3></div>
      ) : (
                ...
                <div>
                  <div className="mb-4 flex items-center gap-4 pr-16">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="truncate block text-sm font-bold text-[#4ade80] mt-1">{projet.artistes?.nom || t('proj.unknown_artist')}</span>
                    </div>
                  </div>
                </div>
                <div className="mt-auto border-t border-gray-800 pt-4">
                  <div className="mb-2 flex items-center justify-between text-sm font-bold">
                    <span className="text-gray-400">{totalSongs} {totalSongs > 1 ? t('proj.tracks_pl') : t('proj.tracks')}</span>
                  ...
                  <button onClick={() => { setActiveProjectId(projet.id); setIsTrackModalOpen(true); }} className="w-full flex items-center justify-center gap-2 rounded-lg bg-gray-900 py-2.5 text-sm font-bold text-white transition-all hover:bg-[#4ade80] hover:text-black border border-gray-800">
                    <ListMusic size={18} /> {t('proj.open_tracklist')}
                  </button>
                </div>
              </div>
            );
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? t('proj.modal.edit') : t('proj.modal.new')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="mb-1 block text-sm font-bold text-gray-400">{t('proj.modal.title_label')}</label><input type="text" required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" /></div>
          <div><label className="mb-1 block text-sm font-bold text-gray-400">{t('proj.modal.artist_label')}</label>
            <select required value={formData.artiste_id} onChange={(e) => setFormData({...formData, artiste_id: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80] [&>option]:bg-black">
              <option value="">{t('proj.modal.select_artist')}</option>{artistes.map(a => <option key={a.id} value={a.id}>{a.nom}</option>)}
            </select>
          </div>
          <div><label className="mb-1 block text-sm font-bold text-gray-400">{t('proj.modal.desc_label')}</label><textarea value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" rows={3} /></div>
          <button type="submit" disabled={isSubmitting} className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-[#4ade80] py-3 font-bold text-black hover:bg-[#4ade80]/90 transition-all">
            {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : (editingId ? t('proj.modal.update_btn') : t('proj.modal.create_btn'))}
          </button>
        </form>
      </Modal>

      {/* MODAL : Tracklist */}
      <Modal isOpen={isTrackModalOpen} onClose={() => setIsTrackModalOpen(false)} title={`${t('proj.tracklist.title')} ${activeProject?.title || ''}`}>
        <div className="space-y-6">
          
          {isAdmin && (
            <form onSubmit={addChanson} className="flex gap-2">
              <input type="text" required value={newSongTitle} onChange={(e)=>setNewSongTitle(e.target.value)} placeholder={t('proj.tracklist.add_placeholder')} className="flex-1 rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" />
          ...
          <div className="max-h-[65vh] overflow-y-auto space-y-4 pr-1 pb-4 custom-scrollbar">
              ...
              <div key={chanson.id} className="group relative overflow-hidden rounded-xl border border-gray-800 bg-gray-900/40 p-5 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all">
                ...
                <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <select value={chanson.status} onChange={(e) => updateChansonStatus(chanson.id, e.target.value)} disabled={!isAdmin} className={`rounded-lg border px-3 py-1.5 text-xs font-bold focus:outline-none shadow-sm [&>option]:bg-gray-900 [&>option]:text-white ${chanson.status === 'TERMINÉ' ? 'border-[#4ade80] bg-[#4ade80]/10 text-[#4ade80]' : chanson.status === 'EN ATTENTE DE CORRECTION DE LA PART DE LARTISTE' ? 'border-orange-500 bg-orange-500/10 text-orange-500' : 'border-gray-700 bg-black text-gray-300'} ${!isAdmin ? 'appearance-none cursor-default' : 'cursor-pointer'}`}>
                    <option value="ENREGISTREMENT">{t('proj.tracklist.status.rec')}</option>
                    <option value="MIXAGE/MASTERING">{t('proj.tracklist.status.mix')}</option>
                    <option value="EN ATTENTE DE CORRECTION DE LA PART DE LARTISTE">{t('proj.tracklist.status.wait')}</option>
                    <option value="TERMINÉ">{t('proj.tracklist.status.done')}</option>
                  </select>
                </div>
                  ...
                  {chanson.fichier_audio ? (
                    <div className="flex flex-col gap-2 rounded-xl border border-gray-800 bg-black p-3 shadow-inner">
                      <div className="flex w-full items-center gap-2">
                        ...
                        {isAdmin && (
                          <button onClick={() => deleteAudio(chanson.id)} className="shrink-0 rounded-lg p-2 text-gray-500 transition-colors hover:bg-red-500/20 hover:text-red-500" title={t('proj.tracklist.delete_audio_title')}>
                            <Trash2 size={20} />
                          </button>
                        )}
                      </div>
                      ...
                      <div className="flex justify-end px-1 mt-1">
                        <button onClick={() => handleAddTimestamp(chanson)} className="flex items-center gap-1.5 text-xs font-bold text-[#4ade80] hover:text-white transition-colors bg-[#4ade80]/10 hover:bg-[#4ade80]/30 px-3 py-1.5 rounded-full border border-[#4ade80]/20">
                          <MapPin size={14} /> {t('proj.tracklist.pin_note')}
                        </button>
                      </div>
                    </div>
                  ) : (
                    isAdmin ? (
                      <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-700 bg-black/50 py-6 transition-all hover:border-[#4ade80] hover:bg-[#4ade80]/5">
                        {uploadingId === chanson.id ? (
                          <><Loader2 size={24} className="mb-2 animate-spin text-[#4ade80]" /><span className="text-sm font-bold text-[#4ade80]">{t('proj.tracklist.uploading')}</span></>
                        ) : (
                          <><UploadCloud size={24} className="mb-2 text-gray-500 transition-colors group-hover:text-[#4ade80]" /><span className="text-sm font-bold text-gray-500 text-center px-4">{t('proj.tracklist.drag_drop')}</span></>
                        )}
                ...
                <div className="mt-2 flex flex-wrap items-center justify-between gap-3 border-t border-gray-800 pt-4">
                  <button onClick={() => toggleRetours(chanson)} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all ${chanson.retours_artiste ? 'bg-orange-500/10 text-orange-500 border border-orange-500/30 hover:bg-orange-500/20 shadow-[0_0_10px_rgba(249,115,22,0.1)]' : 'bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-white'}`}>
                    <MessageSquare size={16} /> {chanson.retours_artiste ? t('proj.tracklist.see_notes') : t('proj.tracklist.add_note')}
                  </button>
                  ...
                  {isAdmin && (
                    <button onClick={() => deleteChanson(chanson.id)} className="flex items-center gap-1.5 rounded px-3 py-1.5 text-xs font-bold text-gray-500 transition-colors hover:bg-red-500/10 hover:text-red-500">
                      <XCircle size={14}/> {t('proj.tracklist.remove_track')}
                    </button>
                  )}
                </div>
                ...
                {activeRetoursId === chanson.id && (
                  <div className="mt-4 animate-in slide-in-from-top-2 rounded-xl bg-black p-4 border border-gray-800 shadow-inner">
                    <label className="mb-2 block text-xs font-bold text-gray-400 uppercase tracking-wider">{t('proj.tracklist.notes_label')}</label>
                    <textarea value={retoursText} onChange={(e) => setRetoursText(e.target.value)} rows={4} placeholder={isAdmin ? t('proj.tracklist.notes_ph_admin') : t('proj.tracklist.notes_ph_artist')} className="w-full rounded-lg border border-gray-700 bg-gray-900 p-3 text-sm font-bold text-white focus:border-orange-500 focus:outline-none transition-colors" />
                    <div className="mt-3 flex justify-end gap-2">
                      <button onClick={() => setActiveRetoursId(null)} className="rounded-lg px-4 py-2 text-xs font-bold text-gray-400 hover:bg-gray-800 hover:text-white transition-colors">{t('proj.tracklist.close')}</button>
                      <button onClick={() => saveRetours(chanson.id)} className="rounded-lg bg-orange-500 px-5 py-2 text-xs font-bold text-black transition-all hover:bg-orange-600">{t('proj.tracklist.save')}</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {activeProject?.chansons?.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 text-gray-600">
                ...
                <p className="font-bold">{isAdmin ? t('proj.tracklist.empty_admin') : t('proj.tracklist.empty_artist')}</p>
              </div>
            )}
          </div>
        </div>
