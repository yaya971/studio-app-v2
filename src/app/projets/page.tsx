...
export default function ProjetsPage() {
  ...
  // NOUVEAU : On va stocker qui est l'utilisateur actuel
  ...
  const fetchData = async () => {
    ...
    let projetsQuery = supabase.from('projets').select('*, artistes(nom)').order('created_at', { ascending: false });
    let artistesQuery = supabase.from('artistes').select('id, nom').order('nom', { ascending: true });

    // 3. LE FILTRE SECRET : Si c'est un artiste, on bloque l'accès aux autres !
    if (loggedInArtiste) {
  ...
  const openNewModal = () => {
    // Si c'est un artiste, on pré-remplit son nom par défaut !
    setFormData({ 
      title: '', 
      description: '', 
      artiste_id: currentArtiste ? currentArtiste.id : '' 
    });
    setEditingId(null);
    setIsModalOpen(true);
  };
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Modifier le projet" : "Nouveau Projet"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {/* Si c'est un artiste, la liste ne contient que lui-même ! */}
