                ...
                return (
                  <div key={projet.id} className="rounded-xl border border-[#a855f7]/30 bg-black/50 p-6 shadow-[0_0_15px_rgba(168,85,247,0.1)]">
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      <span className="text-gray-400 flex items-center gap-1 text-sm font-bold"><Music size={14} /> {totalSongs} Titre{totalSongs > 1 ? 's' : ''}</span>
                    </div>
                    <div className="rounded-lg border border-gray-800 bg-gray-900/50 p-4 space-y-3 text-sm font-bold">
                      {totalSongs === 0 ? (
                        <p className="text-center text-gray-500 italic">Aucun titre ajouté.</p>
                      ) : (
                        <>
                          {terminees > 0 && <div className="flex justify-between items-center"><span className="text-gray-400">Terminées</span><span className="text-[#4ade80] px-2 py-0.5 rounded bg-[#4ade80]/10">{terminees}</span></div>}
                          {enMix > 0 && <div className="flex justify-between items-center"><span className="text-gray-400">En Mix/Mastering</span><span className="text-yellow-500 px-2 py-0.5 rounded bg-yellow-500/10">{enMix}</span></div>}
                          {enAttente > 0 && <div className="flex justify-between items-center"><span className="text-gray-400">En attente de correction</span><span className="text-orange-500 px-2 py-0.5 rounded bg-orange-500/10">{enAttente}</span></div>}
                          {enEnregistrement > 0 && <div className="flex justify-between items-center"><span className="text-gray-400">Enregistrement</span><span className="text-blue-400 px-2 py-0.5 rounded bg-blue-400/10">{enEnregistrement}</span></div>}
                        </>
                      )}
                    </div>
                  </div>
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-400 font-bold">Bienvenue dans votre interface de gestion globale.</p>
      </div>
      ...
      <div className="mb-8 grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-[#4ade80]/30 bg-black/50 p-6 shadow-[0_0_15px_rgba(74,222,128,0.1)]">
          ...
          <p className="text-sm text-gray-400 font-bold">Projets</p><h2 className="text-3xl font-bold text-white">{adminStats.projets}</h2>
        </div>
        <div className="rounded-xl border border-[#4ade80]/30 bg-black/50 p-6 shadow-[0_0_15px_rgba(74,222,128,0.1)]">
          <div className="mb-4 flex justify-between"><Mic2 className="text-[#4ade80]" size={24} /></div>
          <p className="text-sm text-gray-400 font-bold">Sessions</p><h2 className="text-3xl font-bold text-white">{adminStats.sessions}</h2>
        </div>
        <div className="rounded-xl border border-[#4ade80]/30 bg-black/50 p-6 shadow-[0_0_15px_rgba(74,222,128,0.1)]">
          <div className="mb-4 flex justify-between"><Wallet className="text-[#4ade80]" size={24} /></div>
          <p className="text-sm text-gray-400 font-bold">Revenus</p><h2 className="text-3xl font-bold text-[#4ade80]">{adminStats.revenus.toFixed(2)} €</h2>
        </div>
      </div>
