...
export default function ServicesPage() {
  ...
  // Fonction magique pour récupérer le texte dans la bonne langue
  const getLocalizedField = (service: any, field: string) => {
    ...
    return service[field] || ''; // Retourne toujours le français par défaut si la traduction est vide
  };
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    // On envoie le titre localisé pour que la demande soit claire
  ...
  const openEditModal = (service: any = null) => {
    ...
    setFormTab('fr'); // On ouvre toujours sur le Français par défaut
  ...
  // Variables dynamiques pour le formulaire d'édition
  const currentTitleKey = formTab === 'fr' ? 'title' : `title_${formTab}`;
  const currentDescKey = formTab === 'fr' ? 'description' : `description_${formTab}`;
  const currentPriceKey = formTab === 'fr' ? 'price' : `price_${formTab}`;
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {services.map((service, index) => {
          ...
          // Récupération des textes dans la bonne langue
        ...
        {isSent ? (
          ...
          <form onSubmit={handleSendRequest} className="space-y-4">
            <div className="mb-6 flex items-center gap-4 rounded-xl bg-gray-900/50 p-4 border border-gray-800">
              <div>
                <h4 className="font-bold text-white">{selectedService ? getLocalizedField(selectedService, 'title') : ''}</h4>
                <p className="text-sm font-bold text-[#4ade80]">{selectedService ? getLocalizedField(selectedService, 'price') : ''}</p>
              </div>
            </div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title={editingService ? t('srv.modal.edit_title') : t('srv.modal.new_title')}>
        ...
        <form onSubmit={handleSaveService} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-bold text-gray-400">{t('srv.modal.name_label')} ({formTab.toUpperCase()})</label>
            <input type="text" required={formTab === 'fr'} value={(serviceForm as any)[currentTitleKey]} onChange={(e) => setServiceForm({...serviceForm, [currentTitleKey]: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" placeholder={formTab === 'fr' ? "Ex: Mixage Vocal" : ""} />
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-gray-400">{t('srv.modal.desc_label')} ({formTab.toUpperCase()})</label>
            <textarea required={formTab === 'fr'} value={(serviceForm as any)[currentDescKey]} onChange={(e) => setServiceForm({...serviceForm, [currentDescKey]: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" rows={3} />
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-gray-400">{t('srv.modal.price_label')} ({formTab.toUpperCase()})</label>
            <input type="text" required={formTab === 'fr'} value={(serviceForm as any)[currentPriceKey]} onChange={(e) => setServiceForm({...serviceForm, [currentPriceKey]: e.target.value})} className="w-full rounded-lg border border-gray-700 bg-black/50 px-4 py-2 text-white font-bold focus:outline-none focus:border-[#4ade80]" placeholder={formTab === 'fr' ? "Ex: À partir de 50€" : ""} />
          </div>
